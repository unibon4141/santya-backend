...
trait Tables {
   ...
   *  @param imagePath Database column image_path SqlType(VARCHAR), Length(300,true), Default(None)
   *  @param imageBainary Database column image_bainary SqlType(MEDIUMBLOB), Default(None)
   *  @param shopId Database column shop_id SqlType(INT) */
  case class ShopimageRow(shopImageId: Int, imagePath: Option[String] = None, imageBainary: Option[Array[Byte]] = None, shopId: Int)
  /** GetResult implicit for fetching ShopimageRow objects using plain SQL queries */
  implicit def GetResultShopimageRow(implicit e0: GR[Int], e1: GR[Option[String]], e2: GR[Option[Array[Byte]]]): GR[ShopimageRow] = GR{
    prs => import prs._
    ShopimageRow.tupled((<<[Int], <<?[String], <<?[Array[Byte]], <<[Int]))
  }
  ...
  class Shopimage(_tableTag: Tag) extends profile.api.Table[ShopimageRow](_tableTag, Some("demo"), "shopImage") {
    def * = (shopImageId, imagePath, imageBainary, shopId).<>(ShopimageRow.tupled, ShopimageRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(shopImageId), imagePath, imageBainary, Rep.Some(shopId))).shaped.<>({r=>import r._; _1.map(_=> ShopimageRow.tupled((_1.get, _2, _3, _4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column image_path SqlType(VARCHAR), Length(300,true), Default(None) */
    val imagePath: Rep[Option[String]] = column[Option[String]]("image_path", O.Length(300,varying=true), O.Default(None))
    /** Database column image_bainary SqlType(MEDIUMBLOB), Default(None) */
    val imageBainary: Rep[Option[Array[Byte]]] = column[Option[Array[Byte]]]("image_bainary", O.Default(None))
    /** Database column shop_id SqlType(INT) */
    val shopId: Rep[Int] = column[Int]("shop_id")
  }
